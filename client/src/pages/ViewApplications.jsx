...
const ViewApplications = () => {
  ...
  // fuction to update job application status

  const chnageJobapplivationStatus = async (id,status) => {
              ...
              {applicants.filter(item => item.jobId && item.userId).map((applicant,index)=>(
                <tr key={index} className='text-gray-700'>
                  ...
                  <td className='py-2 px-4 border-b'>
                    <a href={applicant.userId.resume} className='bg-blue-50 text-blue-400 px-3 py-1 rounded inline-flex items-center gap-2'  target='_blank'>
                  ...
                  <td className='py-2 px-4 border-b relative'>
                    ...
                    <div className='relative inline-block text-left group'>
                      ...
                      <div className='z-10 hidden absolute right-0 md:left-0 top-0 mt-2 w-32 bg-white border border-gray-200 rounded shadow group-hover:block'>
                        <button onClick={()=> chnageJobapplivationStatus(applicant._id,'Accepted')} className='block w-full text-left px-4 py-2 text-blue-500 hover:gray-100'><b>Accept</b></button>
